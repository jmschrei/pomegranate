...
As a side note, naive Bayes models are Bayes classifiers that assume all features are independent. Older versions of pomegranate explicitly implemented these models. However, neither the prior probabilities nor the likelihood function in Bayes' rule actually care about how complex the model are and so one can just as easily use a hidden Markov model as the model as they can use a simple exponential distribution.

Initialization
--------------

Like all other models in pomegranate, Bayes classifiers can be initialized in one of two ways depending on if you know the parameters of the model beforehand or not. If you do know the parameters you can pass in a list of pre-initialized distributions and immediately use them to perform inference or fine-tune them. If you do not know the parameters then you can pass in a list of distributions initialized without parameters and then subsequently call the ``fit`` method to fit those distributions.
