...
cdef class HiddenMarkovModel( Model ):
	...
	def bake( self, verbose=False, merge="all" ): 
		...
		self.out_transition_pseudocounts = numpy.zeros(
			len( self.graph.edges() ) )
		self.in_transition_log_probabilities_edge_idx = {}
		...
		for a, b, data in self.graph.edges_iter(data=True):
			...
			self.in_transition_log_probabilities_edge_idx[(a, b)] = start
	...
	def train( self, sequences, stop_threshold=1E-9, min_iterations=0,
		...
		for start, end, data in self.graph.edges_iter( data=True ):
			data['weight'] = self.in_transition_log_probabilities[ self.in_transition_log_probabilities_edge_idx[(start, end)] ]
